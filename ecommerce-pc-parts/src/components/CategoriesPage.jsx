...
const CategoriesPage = ({ products: propProducts }) => {
  const { addToCart } = useContext(CartContext);
  const { loading, error } = useContext(ProductContext);
  const [selectedCategory, setSelectedCategory] = useState('Todas');
  const products = propProducts; // Usar los productos pasados como props

  // Depurar los datos disponibles
  useEffect(() => {
    console.log('CategoriesPage - Productos (props):', products);
    console.log('CategoriesPage - Loading:', loading);
    console.log('CategoriesPage - Error:', error);
    console.log('CategoriesPage - Categorías únicas:', [...new Set(products.map((product) => product.category))]);
  ...
  if (products.length === 0) {
    return <div className="container mx-auto p-4 text-[#fff]">No hay productos disponibles.</div>;
  }

  return (
    <div className="container mx-auto p-4">
      <h2 className="text-2xl font-bold mb-4 text-[#fff]">Categorías</h2>
      <div className="mb-4">
